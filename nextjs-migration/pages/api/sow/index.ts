...
async function handleGetProjectSOW(req: NextApiRequest, res: NextApiResponse<SOWData>, projectId: string) {
  try {
    const [poles, drops, fibre] = await Promise.all([
      sql`SELECT * FROM sow_poles WHERE project_id = ${projectId} ORDER BY created_at DESC`,
      sql`SELECT * FROM sow_drops WHERE project_id = ${projectId} ORDER BY created_at DESC`,
      sql`SELECT * FROM sow_fibre WHERE project_id = ${projectId} ORDER BY created_at DESC`
    ]);
  ...
  try {
    ...
    for (const pole of poles) {
      const result = await sql`
        INSERT INTO sow_poles (project_id, pole_number, location, pole_type, height)
        VALUES (${projectId}, ${pole.pole_number}, ${pole.location}, ${pole.pole_type}, ${pole.height})
        RETURNING *
      `;
...
async function handleUploadDrops(req: NextApiRequest, res: NextApiResponse<SOWData>) {
  try {
    ...
    for (const drop of drops) {
      const result = await sql`
        INSERT INTO sow_drops (project_id, drop_number, address, drop_type, cable_length)
        VALUES (${projectId}, ${drop.drop_number}, ${drop.address}, ${drop.drop_type}, ${drop.cable_length})
        RETURNING *
      `;
...
async function handleUploadFibre(req: NextApiRequest, res: NextApiResponse<SOWData>) {
  try {
    ...
    for (const cable of fibre) {
      const result = await sql`
        INSERT INTO sow_fibre (project_id, cable_id, start_location, end_location, cable_type, length)
        VALUES (${projectId}, ${cable.cable_id}, ${cable.start_location}, ${cable.end_location}, ${cable.cable_type}, ${cable.length})
        RETURNING *
      `;
