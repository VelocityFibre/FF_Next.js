...
class AnalyticsDataPopulator {
  ...
  private async clearExistingData() {
    ...
    await neonDb.execute(sql`TRUNCATE TABLE kpi_metrics CASCADE;`);
    await neonDb.execute(sql`TRUNCATE TABLE staff_performance CASCADE;`);
    await neonDb.execute(sql`TRUNCATE TABLE material_usage CASCADE;`);
    await neonDb.execute(sql`TRUNCATE TABLE project_analytics CASCADE;`);
    await neonDb.execute(sql`TRUNCATE TABLE client_analytics CASCADE;`);
    await neonDb.execute(sql`TRUNCATE TABLE financial_transactions CASCADE;`);
  ...
  private async populateStaffPerformance() {
    ...
    await neonDb.execute(staffQuery);
  ...
  private async populateMaterialUsage() {
    ...
    await neonDb.execute(materialQuery);
  ...
  private async populateProjectAnalytics() {
    ...
    await neonDb.execute(projectQuery);
  ...
  private async populateClientAnalytics() {
    ...
    await neonDb.execute(clientQuery);
  ...
  private async populateFinancialTransactions() {
    ...
    await neonDb.execute(financialQuery);
  ...
  private async showSummary() {
    ...
    for (const table of tables) {
      const result = await neonDb.execute(sql.raw(`SELECT COUNT(*) as count FROM ${table};`));
      const count = result[0]?.count || 0;
      console.log(`  ${table}: ${count} records`);
    }
    
    // Show date ranges
    const dateRangeQuery = sql`
      SELECT 
    ...
    const dateRanges = await neonDb.execute(dateRangeQuery);
    console.log('\n📅 Date Ranges:');
    ...
    const metrics = await neonDb.execute(metricsQuery);
    console.log('\n📊 Sample KPI Metrics:');
