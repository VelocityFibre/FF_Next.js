 ...
 * Adapted for FibreFlow Next.js application
 *
 * Purpose: Import daily OneMap CSV files with comprehensive verification
 * - Compares new CSV against existing database
 * - Performs multiple verification checks
 * - Generates detailed change reports
 * - Prevents duplicate imports
 * - Tracks status changes and history
 *
 * Usage: node scripts/import-onemap-daily.js <csv-file-path>
 */
...
async function isFirstInstanceForPole(poleNumber, normalizedStatus, statusDate) {
  ...
  const poleRecords = await sql`
    SELECT current_data
    FROM ${sql(STAGING_TABLE)}
    WHERE current_data->>'Pole Number' = ${poleNumber}
  `;

  let isFirst = true;

  for (const row of poleRecords) {
    const data = row.current_data;
    const recordStatus = normalizeStatus(data.Status);
    const recordDate = data.date_status_changed;
...
async function processRecord(record, report, batchId, firstInstanceTracking) {
  ...
  await sql`
    INSERT INTO ${sql(STAGING_TABLE)} (
      ...
      ${propertyId}, ${stagingRecord.tracking_key}, ${JSON.stringify(record)},
      ${batchId}, ${stagingRecord.first_seen_date}, ${stagingRecord.last_updated_date},
      ${stagingRecord.version}, ${stagingRecord.is_active}
    )
  `;
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: node scripts/import-onemap-daily.js <csv-file-path>');
    process.exit(1);
  }
  ...
  try {
    ...
    await sql`
      INSERT INTO ${sql(IMPORT_BATCHES_TABLE)} (
        batch_id, import_date, file_name, status
      ) VALUES (
        ${batchId}, ${new Date()}, ${path.basename(csvPath)}, 'processing'
      )
    `;
